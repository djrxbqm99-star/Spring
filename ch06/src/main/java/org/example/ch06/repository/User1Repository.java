...
// Repository : JPA 에서 데이터 엑세스 객체(DAO)
@Repository
public interface User1Repository extends JpaRepository<User1, String> {
    ...
    // SELECT * FROM User1 WHERE userid = {userid} AND name = {name}
    public User1 findByUseridAndName(String userid, String name);

    // SELECT * FROM User1 WHERE userid = {userid} OR name = {name}
    public List<User1> findByUseridOrName(String userid, String name);
    ...
    // SELECT * FROM User1 WHERE age >= {low} AND age <= {high}
    public List<User1> findByAgeBetween(int low, int high);
    ...
    public List<User1> findByNameStartsWith(String name); // SELECT * FROM User1 WHERE name
    ...
    @Query("SELECT p, c FROM Parent AS p " +
            ...
            "WHERE p.pid = :pid")
    public Object[] selectParentChild(String pid);
     */
}
